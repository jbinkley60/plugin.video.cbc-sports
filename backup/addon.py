...
def INDEX(url):
	...
	for item in jdata['schedule']:
		...
		try:
			...
			xbmc.log('CBC Sports Event has a bad date/time to calculate duration. - ' + str(title) + ' '  + starttime, xbmc.LOGINFO)		
		except:
			iduration = 0
			xbmc.log('CBC Sports Event has a bad date/time to calculate duration. - ' + str(title), xbmc.LOGINFO)
